...
- **Unit** — the class in isolation. Every collaborator it is given is a mock or a fake. No database, no network,
  no filesystem, no application-framework context.
- **Integration** — the class against the **real** thing it integrates with: a database, a cache, an object
  ...
  A stub server standing in for a third-party HTTP API counts as real. A mocked collaborator does not.
- **System** — the fully wired application, **nothing mocked**, entered the way production enters it: an HTTP
  ...
  by calling an internal method directly.
...
- **Performance** — the whole application, entered as production does, driven under the load the scenario states
  ...
  faked. The test carries the threshold in code and is red past it. The step that runs it records the figure
  rather than gating on it. Red-first does not apply. It runs where and how the module's testing conventions
  say, never with every build.
...
such mapping cannot be planned: ask for it before writing steps. Never invent one from the package names.

Do **not** copy the mapping into the plan file.
...
goes back to the design as a new `DN` entry, never in here.
...
already covered by existing tests — even if they are related to the changed code.
...
- **Per method** — one method, one outcome, read off that method's body. For a change no premise derives: a
  test to delete, an assertion a new field needs, a case a grown enum adds. A `— delete` names the method.
- **Whole class** — `every test in this class — delete`, for a class rewritten from nothing: every method the
  ...
  method. A class keeping even one test is per-method.
- **Per class (premise)** — a fact about the change, and what it implies for a test that meets it: `two ports
  ...
  verifies the one mock`. The premise names the collaborator, the field or the value it turns on, so that
  reading a test's body settles whether it meets it. Never a transformation to work out per method with the
  premise left implicit, and never one sentence stretched over methods it does not all hold for. Name a method
  under a premise bullet only as an example.
...
**Removed behaviour is searched for, not remembered.** When a decision removes anything a test can observe — a
response field, a status, a component of a record, a value a message carries — search the test tree for that
value and list every hit as an `update:` bullet, whatever class it lands in. The search term is the removed
thing itself, not the classes the plan happens to touch.

**What the change removed gets no scenario of its own.** A retired path, a dropped field, a command that no
longer exists: do not list a scenario asserting that the old one now refuses. What proves the removal is the
scenarios on what replaced it, plus the `update:` bullets above that strip the old thing out of the tests that
named it.
...
point*, not for the classes in the step list, and list every hit as an `update:` bullet.
...
over every possible outcome. Where the design log answered a finding by reading the dependency rather than
running it, the step agent observes the real behaviour before writing the assertion. The module's testing
conventions own this rule.
...
The tests are expected to **fail at runtime** against the stabilization phase's stubs.
**No production implementation is done in this section.**
Steps are small and focused — one class, one concern.
**Every dependency the class is handed is mocked or faked.** A test here never touches real infrastructure.
...
Tests are expected to **fail at runtime** against the stub. **No implementation is done in this section.**
...
- `after:` (optional) — the IDs of other green-phase steps whose classes sit on this one's real execution path.
  A mapper or a domain object implemented by another green step is a real dependency. Same rule as the unit
  format: only real, unmocked collaborators, never mocked ones.
...
failure.** The step never tunes production code to reach it, and the pipeline never blocks on it.
