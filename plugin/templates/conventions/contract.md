...
| **Performance**: the tool, where the figure is measured, the command that runs it and keeps it out of the full suite, where a test lives             | `plan-task`, `review-plan`, the performance step, every close that reruns a test | not measured: a measurable scenario gets no step, and the plan says so                                                                       |
