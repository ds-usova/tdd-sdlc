...
- **the skipped count is back to the baseline, plus the reproductions the Run Log names** — every test
  ...
  step that silently skipped its own `update:` bullets. The reproductions the Run Log names by class and
  method are the only skips allowed above the baseline. Compare against
  the baseline's number rather than against zero: a module whose infrastructure skips on its own (no container
  runtime, say) starts above zero and must return there, not below it;
