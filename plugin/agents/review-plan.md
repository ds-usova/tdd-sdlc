...
assumptions at face value. Read the repository (test files, production code, conventions files), not just the
plan file text.
...
- Confirm the **Step-by-Step Implementation Map** nests correctly: the four `### <Group>` headings — Stabilization,
  ...
  heading sits under the group the `plan-task` skill assigns it to (e.g. no `TDD Unit Red Phase` section
  floating outside the Red Phase group, no stabilization section appearing under Green Phase).
...
Give one block per finding, numbered from `1` for this report alone. Never an `RF` number: the session that owns
the plan assigns those.
