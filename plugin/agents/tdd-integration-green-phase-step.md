...
integration level. The step comes in the same two variants as the red step:
...
- **Against the framework** — the framework calls the class, and the tests reach it through the protocol.
  ...
  the documented responses. Whatever it hands the work to stays mocked.

The tests are the specification — **do not create or modify any test code**, and never implement what your class
delegates to.

Stay inside your own step: your class is yours alone.
...
A stack-specific decision the conventions and the existing implementations you read do not cover (no mapping
style recorded anywhere, say) is a blocker. Report it. Do not introduce a new tool or pattern.
...
3. Framework variant only: read the module's **schema** for that entry point. Take the validation constraints and
   response contracts the implementation must satisfy from it. The schema is read-only for this step (see the
   GREEN guardrail).
4. Read the types, mappers, and other collaborators the class uses.
5. Read one or two neighbouring implementations of the same variant as a style reference. Match their structure,
   error handling and mapping idiom.
...
- Implement **only stub methods the test class covers**. A stub with no covering test stays a stub. Record it in
  your report.
- Follow the conventions' production-code style and the variant-specific idioms the conventions define.
...
3. Iterate on the implementation until **every test in the class passes**, including tests that were already
   green before this step. Fix a test this step broke before the step is done.

   In the framework variant, many tests may already be green when the step starts. Make the whole class green,
   however much of the distance is left. A near-no-op step still verifies and reports.
4. Confirm the GREEN guardrail: green is reached **only by changing the target class** —
    ...
    - never by editing contract artifacts: a schema and database migrations are **stabilization property**.
      A test failing on a missing table or a missing schema constraint is a blocker;
...
- Only modify the target class. A missing collaborator or an absent mapper is a blocker to report, never to
  patch around.
- Never modify test code, test infrastructure configuration, contract artifacts (schemas, migrations), other
  production classes, other agents' targets, or the plan file.
