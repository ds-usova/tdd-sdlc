...
Each `<id>.jsonl` is one agent transcript the recorder hook is fed, copied to `subagents/agent-<id>.jsonl`; an `<id>.meta.json` beside it goes with it. Every one is a user prompt and one assistant turn; the prompt is what varies.
...
The first four are in the golden; the last two are appended after it is checked.
