...
p="$(bad_plan covers-prose)"
check_match "validate: a covers: written as prose" "^RU02's covers: is not a list of backticked entries" "$(validate_bad "$p")"
check_match "validate: a mocks: written as prose" "^RI02's mocks: is not 'none' or a list of backticked names" "$(validate_bad "$p")"
check_no_match "validate: a component entry point is code" "^RI02's covers:" "$(validate_bad "$p")"
check_match "validate: a red step without covers:" "^RS01 has no 'covers:'" "$(validate_bad "$p")"
check_no_match "validate: a backticked covers: is not reported" "^RU01" "$(validate_bad "$p")"
p="$(bad_plan plan-shape)"
check_match "validate: a section the plan does not have" "^'## Components' \(line [0-9]+\) is not a plan section" "$(validate_bad "$p")"
check_match "validate: a missing Spec line" '^no \*\*Spec:\*\* line' "$(validate_bad "$p")"
check_match "validate: a section out of order" "^'#### TDD Unit Red Phase' \(line [0-9]+\) is out of order under Red Phase" "$(validate_bad "$p")"
check_match "validate: a step under another type's section" "^RS01 sits under 'Red Phase / TDD Unit Red Phase'" "$(validate_bad "$p")"
check_match "validate: a green step waiting on a red one" "^GI02's after: names RU03" "$(validate_bad "$p")"
check_match "validate: a red step with no green pair" '^RU03 has no green step' "$(validate_bad "$p")"
check_match "validate: a green step with no red pair" '^GU03 has no red step' "$(validate_bad "$p")"
check_no_match "validate: a paired step is not reported" '^RU01 has no green' "$(validate_bad "$p")"
p="$(bad_plan spec-missing)"
check_match "validate: a Spec link to nothing" '^\*\*Spec:\*\* links nowhere.md, which does not exist' "$(validate_bad "$p")"
p="$(bad_plan spec-open)"
check_match "validate: a spec still open" '^the spec is not settled' "$(validate_bad "$p")"
check_match "validate: names the open decision" 'DN02' "$(validate_bad "$p")"
